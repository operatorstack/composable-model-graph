...
The final path is checked against ground truth with `exact_match_evaluator`.
`default_feedback_resolver` turns failure into `retry` and success into
`accept`. `compare_runs` then reports the winning configuration,
evaluation-score delta, `pathScore` signal delta, and the first trace step where
the runs differ.
